/**
 * One-off script: add 'pms' entitlement to all dev tenants that don't have it yet.
 * Safe to run multiple times (ON CONFLICT DO NOTHING).
 *
 * Usage: npx tsx tools/scripts/add-pms-entitlement.ts
 */
...
async function main() {
  ...
  for (const tenant of allTenants) {
    ...
    `);
    console.log(`✓ pms entitlement added (or already exists) for tenant: ${tenant.name}`);
  }
