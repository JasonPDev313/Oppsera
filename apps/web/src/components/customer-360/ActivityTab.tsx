  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex gap-3 rounded-lg border border-gray-200 bg-surface p-4">
...
function FeedItem({ item }: { item: ActivityFeedItem }) {
  ...
  return (
    <div className="flex gap-3 rounded-lg border border-gray-200 bg-surface p-4 transition-colors hover:border-gray-300">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="shrink-0 text-xs text-gray-400" title={formatDateTime(item.createdAt)}>
            {formatRelativeTime(item.createdAt)}
          </span>
        </div>
        {item.details && (
          <p className="mt-1 text-sm text-gray-600 line-clamp-2">{item.details}</p>
        )}
        {item.createdBy && (
          <p className="mt-1 text-xs text-gray-400">by {item.createdBy}</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div
      className={`rounded-lg border p-4 transition-colors ${
        note.isPinned ? 'border-amber-200 bg-amber-50/30' : 'border-gray-200 bg-surface'
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        {isEditing ? (
      ...
      <div>
        ...
        {feedLoading && !feedData ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-lg border border-gray-200 bg-surface px-6 py-12 text-center">
            ...
            <p className="mb-4 text-sm text-gray-600">Failed to load activity feed.</p>
            <button
              type="button"
              onClick={refreshFeed}
