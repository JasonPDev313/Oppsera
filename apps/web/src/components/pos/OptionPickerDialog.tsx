...
export function OptionPickerDialog({ open, onClose, item, onAdd }: OptionPickerDialogProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="option-picker-dialog-title">
        ...
        <div className="max-h-[60vh] overflow-y-auto px-6 py-4 space-y-6">
          ...
          {item.isTrackInventory && item.onHand !== null && (
            <p className="text-sm text-muted-foreground">
              Stock: <span className="font-medium text-foreground">{item.onHand}</span> at this
              location
            </p>
          )}
        </div>
