    ...
    <div ref={containerRef} className={`relative ${className ?? ''}`}>
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-gray-200 bg-surface py-2.5 pl-10 pr-10 text-sm text-gray-900 placeholder-gray-400 outline-none transition-colors focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 dark:border-gray-700 dark:text-gray-100 dark:placeholder-gray-500"
        />
        {query ? (
          <button
            type="button"
          ...
          <kbd className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rounded border border-gray-200 bg-gray-50 px-1.5 py-0.5 text-[10px] font-medium text-gray-400 dark:border-gray-600 dark:bg-gray-800">
            /
          </kbd>
        )}
      </div>
      ...
      {isOpen && flatResults.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-40 mt-1 max-h-80 overflow-y-auto rounded-lg border border-gray-200 bg-surface shadow-lg dark:border-gray-700">
          {sections.map((section) => (
            <div key={section.type}>
              <div className="flex items-center gap-1.5 border-b border-gray-100 px-3 py-1.5 dark:border-gray-700/50">
      ...
      {isOpen && flatResults.length === 0 && isLoading && (
        <div className="absolute left-0 right-0 top-full z-40 mt-1 rounded-lg border border-gray-200 bg-surface px-3 py-4 shadow-lg dark:border-gray-700">
