  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <SummaryCard label="Total" value={summary.totalRows} color="text-gray-700 dark:text-gray-300" />
        <SummaryCard label="Will Create" value={summary.createCount} color="text-green-600 dark:text-green-400" icon={UserPlus} />
        <SummaryCard label="Will Update" value={summary.updateCount} color="text-blue-600 dark:text-blue-400" icon={UserCog} />
        <SummaryCard label="Skipped" value={summary.skipCount} color="text-gray-500" icon={UserX} />
        <SummaryCard label="Errors" value={summary.errorRows} color="text-red-600 dark:text-red-400" icon={XCircle} />
      </div>

      {/* Warnings */}
      {(summary.distinctRolesUnmapped.length > 0 || summary.distinctLocationsUnmapped.length > 0 || summary.duplicateEmailsInFile.length > 0) && (
        <div className="rounded-lg border border-yellow-500/40 bg-yellow-500/10 px-4 py-3 space-y-1 text-sm text-yellow-700 dark:text-yellow-400">
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 bg-white dark:bg-gray-900">
            <tr className="border-b border-gray-200 dark:border-gray-700 text-left">
              <th className="pb-2 pr-3 font-medium text-gray-500 dark:text-gray-400 w-10">#</th>
              <th className="pb-2 pr-3 font-medium text-gray-500 dark:text-gray-400 w-16">Action</th>
              <th className="pb-2 pr-3 font-medium text-gray-500 dark:text-gray-400">Name</th>
              <th className="pb-2 pr-3 font-medium text-gray-500 dark:text-gray-400">Email</th>
              <th className="pb-2 pr-3 font-medium text-gray-500 dark:text-gray-400">Username</th>
              <th className="pb-2 pr-3 font-medium text-gray-500 dark:text-gray-400">Status</th>
              <th className="pb-2 font-medium text-gray-500 dark:text-gray-400">Issues</th>
            </tr>
          </thead>
            ...
            {displayRows.map((row) => {
              ...
              return (
                <tr key={row.rowNumber} className={!row.isValid ? 'bg-red-500/5' : ''}>
                  ...
                  <td className="py-2 pr-3 text-gray-600 dark:text-gray-400 text-xs">{row.email || '—'}</td>
                  <td className="py-2 pr-3 text-gray-600 dark:text-gray-400 text-xs font-mono">{row.username || '—'}</td>
                  <td className="py-2 pr-3">
      ...
      <div className="flex justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <div className="flex gap-3">
          <button
            onClick={onDryRun}
            disabled={isLoading}
            className="flex items-center gap-2 px-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 disabled:opacity-50"
          >
