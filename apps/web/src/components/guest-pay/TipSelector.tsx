...
export function TipSelector({ tipSettings, baseCents, selectedTipCents, onTipChange }: TipSelectorProps) {
  ...
  return (
    <div>
      <h3 className="text-sm font-semibold text-foreground mb-3">Add a Tip</h3>
      ...
      {selectedTipCents > 0 && (
        <div className="mt-3 text-center">
          <span className="text-sm text-muted-foreground">
            Tip: <span className="font-semibold text-foreground">${(selectedTipCents / 100).toFixed(2)}</span>
          </span>
        </div>
      )}
    </div>
