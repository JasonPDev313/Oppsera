      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium text-indigo-600 transition-colors hover:bg-indigo-500/10"
          >
            ...
            {cards.map((method) => (
              <div key={method.id}>
                ...
                {confirmRemove === method.id && (
                  <div className="mt-1 flex items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs">
                    <span className="text-red-500">Remove this card?</span>
                    <button
                      type="button"
                      onClick={() => handleRemoveCard(method.id)}
                      disabled={isActing}
                      className="font-medium text-red-500 hover:text-red-400 disabled:opacity-50"
                    >
                      Confirm
                    </button>
                    <button
          ...
          <div className="space-y-2">
            {bankAccounts.map((method) => (
              <div key={method.id}>
                ...
                {confirmRemove === method.id && (
                  <div className="mt-1 flex items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs">
                    <span className="text-red-500">Remove this bank account?</span>
                    <button
                      type="button"
        ...
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border py-8">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={() => setShowAddBank(true)}
              className="rounded-lg border border-emerald-500/30 px-3 py-1.5 text-xs font-medium text-emerald-500 transition-colors hover:bg-emerald-500/10"
            >
              Add Bank Account
            </button>
          </div>
