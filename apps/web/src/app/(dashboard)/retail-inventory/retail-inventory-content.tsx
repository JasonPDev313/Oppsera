...
export default function RetailInventoryContent() {
  ...
  const { data: items, isLoading, hasMore, loadMore, mutate } = useCatalogItems({
    ...
    itemTypes: 'retail,green_fee,rental,other',
  ...
  const openDrawer = useCallback(
    (row: EnrichedRow) => {
      itemEditDrawer.open(row.id, { mode: 'retail', onSaveSuccess: () => mutate() });
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">Retail Inventory</h1>
        <button
          type="button"
          onClick={() => itemEditDrawer.open('new', { mode: 'retail', onSaveSuccess: () => mutate() })}
      ...
      {!isLoading && enrichedItems.length === 0 && !search && !catId && !lowStockOnly ? (
        <EmptyState
          icon={Package}
          title="No retail items yet"
          description="Add your first retail or package item to get started"
          action={{ label: 'Add Retail Item', onClick: () => itemEditDrawer.open('new', { mode: 'retail', onSaveSuccess: () => mutate() }) }}
        />
      ) : (
        <>
          <DataTable
