...
export default function FnbInventoryContent() {
  ...
  const openDrawer = useCallback(
    (row: EnrichedRow) => {
      itemEditDrawer.open(row.id, { mode: 'fnb', onSaveSuccess: () => mutate() });
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => itemEditDrawer.open('new', { mode: 'fnb', onSaveSuccess: () => mutate() })}
      ...
      {!isLoading && enrichedItems.length === 0 && !search && !catId && !lowStockOnly ? (
        <EmptyState
          icon={Package}
          title="No F&B items yet"
          description="Add your first food or beverage item to get started"
          action={{ label: 'Add F&B Item', onClick: () => itemEditDrawer.open('new', { mode: 'fnb', onSaveSuccess: () => mutate() }) }}
        />
      ) : (
        <>
          <DataTable
      ...
      >
        <div className="mt-4">
          <label htmlFor="fnb-deactivate-reason" className="block text-sm font-medium text-foreground mb-1">
            Reason (optional)
          </label>
          <textarea
            id="fnb-deactivate-reason"
            ...
            rows={3}
            placeholder="e.g., No longer on menu"
          />
        </div>
      </ConfirmDialog>
