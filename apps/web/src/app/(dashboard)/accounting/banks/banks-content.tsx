          ...
          {banks.map((bank) => (
            <div
              ...
              className={`relative rounded-lg border bg-surface p-5 space-y-3 ${
                bank.isActive ? 'border-gray-200' : 'border-gray-100 opacity-60'
              }`}
            >
              {bank.isDefault && (
                <div className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-amber-100">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {bank.bankName && (
                      <p className="text-xs text-gray-500">{bank.bankName}</p>
                    )}
                  </div>
                </div>
                <button
              ...
              {bank.accountNumberLast4 && (
                <p className="text-sm text-gray-500">
                  Account ending in <span className="font-mono font-medium text-gray-700">••••{bank.accountNumberLast4}</span>
                </p>
              )}

              <div className="rounded-lg bg-gray-50 p-2">
                <p className="text-xs text-gray-500">Linked GL Account</p>
                <p className="text-sm text-gray-900">
                  {bank.glAccountNumber && (
                    <span className="mr-1.5 font-mono text-xs text-gray-500">{bank.glAccountNumber}</span>
                  )}
      ...
      <div className="relative z-10 w-full max-w-md rounded-lg border border-gray-200 bg-surface p-6 shadow-xl space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            />
            {errors.glAccountId && <p className="mt-1 text-xs text-red-600">{errors.glAccountId}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="1234"
              maxLength={4}
              className="w-24 rounded-lg border border-gray-300 px-3 py-2 text-sm font-mono focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            />
            {errors.accountNumberLast4 && <p className="mt-1 text-xs text-red-600">{errors.accountNumberLast4}</p>}
          </div>
