      ...
      {!isLoading && rows.length > 0 && (
        <div className="overflow-hidden rounded-lg border border-gray-200 bg-surface">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-gray-500">Total Debits</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-gray-500">Total Credits</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-gray-500">Net Balance</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => (
                  <tr key={row.groupLabel} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">{row.groupLabel}</td>
                    <td className="px-4 py-3 text-right text-sm tabular-nums text-gray-900">{formatAccountingMoney(row.totalDebits)}</td>
                    <td className="px-4 py-3 text-right text-sm tabular-nums text-gray-900">{formatAccountingMoney(row.totalCredits)}</td>
                    <td className="px-4 py-3 text-right text-sm font-medium tabular-nums text-gray-900">{formatAccountingMoney(row.netBalance)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-gray-300 bg-gray-50 font-bold">
                  <td className="px-4 py-3 text-sm text-gray-800">Grand Total</td>
                  <td className="px-4 py-3 text-right text-sm tabular-nums text-gray-900">{formatAccountingMoney(totalDebits)}</td>
                  <td className="px-4 py-3 text-right text-sm tabular-nums text-gray-900">{formatAccountingMoney(totalCredits)}</td>
                  <td className="px-4 py-3 text-right text-sm tabular-nums text-gray-900">{formatAccountingMoney(totalNet)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          <div className="space-y-3 p-4 md:hidden">
            {rows.map((row) => (
              <div key={row.groupLabel} className="rounded-lg border border-gray-100 p-3 space-y-1">
                <p className="text-sm font-medium text-gray-900">{row.groupLabel}</p>
