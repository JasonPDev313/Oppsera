...
export default function ArAgingReportContent() {
  const [asOfDate, setAsOfDate] = useState(
    () => new Date().toISOString().split('T')[0]!,
  );
  const [searchTerm, setSearchTerm] = useState('');

  const { data: rawData, isLoading, mutate } = useARAging({ asOfDate });

  // The hook returns the full report object cast as ARAgingRow[]
  const report = useMemo<AgingReport>(() => {
    const d = rawData as unknown;
    if (d && typeof d === 'object' && 'rows' in (d as Record<string, unknown>)) {
      return d as AgingReport;
    }
    // Fallback: if data is actually an array (unlikely but safe)
    ...
    return { asOfDate, rows: arr, totals };
  }, [rawData, asOfDate]);
  ...
  const filtered = useMemo(() => {
    ...
    return report.rows.filter(
      (r) =>
        ...
        r.customerId.toLowerCase().includes(lc),
    );
  ...
  const overdueCount = report.rows.filter(
    (r) => r.days1to30 + r.days31to60 + r.days61to90 + r.days90plus > 0,
  ).length;

  const exportUrl = `/api/v1/ar/reports/aging${buildQueryString({ asOfDate, format: 'csv' })}`;

  return (
    <AccountingPageShell
      title="AR Aging Report"
      breadcrumbs={[{ label: 'Reports', href: '/accounting/reports' }, { label: 'AR Aging' }]}
    >
      {/* Print header */}
      <div className="hidden print:block print:mb-4">
        <h1 className="text-xl font-bold text-foreground">Accounts Receivable Aging Report</h1>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <KPICard
              icon={Users}
              label="Customers"
              value={`${report.rows.length} (${overdueCount} overdue)`}
          ...
          {report.totals.total > 0 && (
            ...
            >
              {pastDueTotal > 0
                ? `${formatAccountingMoney(pastDueTotal)} past due across ${overdueCount} customer${overdueCount !== 1 ? 's' : ''}`
                : 'All receivables are current — no past due amounts.'}
            </div>
          )}
          ...
          <div className="hidden print:block print:mt-6 print:border-t print:border-gray-300 print:pt-2">
            <p className="text-xs text-muted-foreground">
              Generated {new Date().toLocaleDateString()} — AR Aging Report — As of {asOfDate}
            </p>
          </div>
        </>
      )}
