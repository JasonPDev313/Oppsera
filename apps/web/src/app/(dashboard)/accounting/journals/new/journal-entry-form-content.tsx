      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="rounded-lg border border-gray-200 bg-surface p-5 space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              />
              {errors.businessDate && (
                <p className="mt-1 text-xs text-red-600">{errors.businessDate}</p>
              )}
            </div>
            <div>
        ...
        <div className="space-y-3">
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-surface">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500 w-[280px]">Account</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500 w-[150px]">Memo</th>
                    <th className="px-3 py-2 text-right text-xs font-medium uppercase tracking-wide text-gray-500 w-[130px]">Debit</th>
                    <th className="px-3 py-2 text-right text-xs font-medium uppercase tracking-wide text-gray-500 w-[130px]">Credit</th>
                    ...
                    <tr key={line.key} className="border-b border-gray-100 last:border-0">
                      <td className="px-3 py-2">
                        ...
                        {errors[`line_${line.key}_account`] && (
                          <p className="mt-0.5 text-xs text-red-600">{errors[`line_${line.key}_account`]}</p>
                        )}
                      </td>
                      <td className="px-3 py-2">
                        <input
            ...
            <div className="space-y-4 p-4 md:hidden">
              {lines.map((line, idx) => (
                <div key={line.key} className="rounded-lg border border-gray-100 p-3 space-y-3">
                  <div className="flex items-center justify-between">
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex gap-6">
                <div>
                  <span className="text-xs font-medium uppercase tracking-wide text-gray-500">Total Debits</span>
                  <p className="text-lg font-semibold tabular-nums text-gray-900">
                    ${totalDebits.toFixed(2)}
                  </p>
                </div>
                <div>
                  <span className="text-xs font-medium uppercase tracking-wide text-gray-500">Total Credits</span>
                  <p className="text-lg font-semibold tabular-nums text-gray-900">
        ...
        <div className="flex flex-col gap-3 border-t border-gray-200 pt-4 sm:flex-row sm:justify-end">
          ...
          <button
            ...
            disabled={isSaving}
            className="rounded-lg border border-indigo-300 px-6 py-2.5 text-sm font-medium text-indigo-700 hover:bg-indigo-50 disabled:opacity-50"
          >
