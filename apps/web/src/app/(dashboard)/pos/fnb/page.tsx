    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-lg bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 pt-6 pb-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Recall Ticket</h3>
            <p className="mt-0.5 text-sm text-gray-500">
              {heldOrderCount} held {heldOrderCount === 1 ? 'ticket' : 'tickets'}
            </p>
          </div>
          <button
            type="button"
          ...
          {heldOrders.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="mt-3 text-sm text-gray-500">
                No held tickets found for this terminal
              </p>
              <p className="mt-1 text-xs text-gray-400">
                Tickets held on this terminal will appear here
              </p>
            </div>
          ) : (
...
export default function FnbPOSPage() {
  ...
  const lastLine = useMemo(() => {
    ...
    // Get the line with the highest sortOrder (most recently added)
    return [...lines].sort((a, b) => b.sortOrder - a.sortOrder)[0];
  ...
  const handleRepeatLast = useCallback(() => {
    ...
    const input: AddLineItemInput = {
      ...
      qty: lastLine.qty,
      ...(lastLine.modifiers && lastLine.modifiers.length > 0 && {
        modifiers: lastLine.modifiers.map((mod) => ({
      ...
      ...(lastLine.specialInstructions && {
        specialInstructions: lastLine.specialInstructions,
      }),
      ...(lastLine.selectedOptions &&
        Object.keys(lastLine.selectedOptions).length > 0 && {
          selectedOptions: lastLine.selectedOptions,
        }),
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex w-[60%] flex-col border-r border-gray-200 bg-white">
          ...
          <div className="flex shrink-0 items-center gap-2 border-b border-gray-100 px-4 py-2">
            ...
            {viewMode === 'catalog' && catalog.breadcrumb.length > 0 && (
          ...
          {viewMode === 'quick' ? (
            ...
            /* ── Catalog View ──────────────────────────────────────── */
            <>
              {/* Two-column body: CategoryRail (optional) + Large Items Grid */}
              <div className="flex flex-1 overflow-hidden">
          ...
          {/* Search bar at bottom of left panel (less prominent for F&B) */}
        ...
        <div className="flex w-[40%] flex-col bg-white">
          ...
          <div className="shrink-0 border-b border-gray-200 px-4 py-3">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-bold text-gray-900">
                {orderNumber ? `Ticket #${orderNumber}` : 'Ticket'}
              </h2>
              <CustomerAttachment
          ...
          <div className="shrink-0 px-4 py-2">
            <button
              ...
              disabled={!lastLine}
              className="flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 px-3 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <Copy className="h-4 w-4" />
              Repeat Last
              {lastLine && (
                <span className="ml-1 max-w-[120px] truncate text-gray-500">
                  ({lastLine.catalogItemName})
                </span>
              )}
            </button>
          </div>
          ...
          <div className="shrink-0 border-t border-gray-100 px-4 py-2">
            <div className="flex gap-2">
              <button
                ...
                disabled={!hasItems}
                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-gray-300 px-2 py-2.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
              ...
              <button
                ...
                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-gray-300 px-2 py-2.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-50"
              >
            ...
            <div className="relative w-full max-w-sm rounded-lg bg-white shadow-xl">
              <div className="border-b border-gray-200 px-6 pt-6 pb-4">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  This will void ticket {pos.currentOrder?.orderNumber}. This action cannot be undone.
                </p>
              </div>
              <div className="px-6 py-4">
                <label htmlFor="fnb-void-reason" className="block text-sm font-medium text-gray-700">
                  Reason
                </label>
                <input
                  id="fnb-void-reason"
      ...
      <ConfirmDialog
        ...
        title="End Shift"
        description="Are you sure you want to close the current shift? Make sure all tickets are settled before closing."
