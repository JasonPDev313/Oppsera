...
export default function InvoiceDetailContent() {
  ...
  return (
    ...
    >
      ...
      <div className="rounded-lg border border-border bg-surface p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-foreground">{invoice.customerName ?? '—'}</p>
          </div>
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Invoice Date</p>
            <p className="mt-1 text-sm text-foreground">{invoice.invoiceDate}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm capitalize text-foreground">{invoice.sourceType.replace('_', ' ')}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Total</p>
            <p className="mt-1 text-lg font-bold tabular-nums text-foreground">{formatAccountingMoney(invoice.totalAmount)}</p>
          </div>
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Tax</p>
            <p className="mt-1 text-lg font-bold tabular-nums text-foreground">{formatAccountingMoney(invoice.taxAmount)}</p>
          </div>
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Balance Due</p>
            <p className={`mt-1 text-lg font-bold tabular-nums ${Number(invoice.balanceDue) > 0 ? 'text-red-500' : 'text-green-500'}`}>
      ...
      {/* Lines */}
      <div>
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Invoice Lines</h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-muted">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-muted-foreground">Unit Price</th>
                ...
                {invoice.lines.map((line) => (
                  <tr key={line.id} className="border-b border-border last:border-0">
                    <td className="px-4 py-3 text-sm text-foreground">
                      {line.revenueAccountNumber && <span className="mr-1 font-mono text-xs text-muted-foreground">{line.revenueAccountNumber}</span>}
                    ...
                    <td className="px-4 py-3 text-right text-sm tabular-nums text-foreground">{formatAccountingMoney(line.unitPrice)}</td>
              ...
              <tfoot>
                <tr className="border-t border-border bg-muted font-semibold">
                  <td colSpan={4} className="px-4 py-3 text-sm text-foreground">Total</td>
                  <td className="px-4 py-3 text-right text-sm tabular-nums text-foreground">{formatAccountingMoney(invoice.totalAmount)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
            ...
            {invoice.lines.map((line) => (
              <div key={line.id} className="rounded border border-border p-3 space-y-1">
                <div className="flex justify-between text-sm">
                  <span className="text-foreground">{line.revenueAccountName ?? line.revenueAccountId}</span>
      ...
      {showVoidDialog && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-lg border border-border bg-surface p-6 shadow-xl space-y-4">
            <h3 className="text-lg font-semibold text-foreground">Void Invoice</h3>
            <p className="text-sm text-muted-foreground">This will create a reversal GL entry. This cannot be undone.</p>
            <div>
              <label htmlFor="ar-invoice-void-reason" className="block text-sm font-medium text-foreground mb-1">Reason <span className="text-red-500">*</span></label>
              <textarea
                id="ar-invoice-void-reason"
                value={voidReason}
