...
export async function GET() {
  ...
  try {
    ...
    const stuckCount = Number(stuck[0]?.cnt ?? 0);
    if (stuckCount > 0) {
      status = 'degraded';
      warnings.push(`${stuckCount} connection(s) stuck in idle-in-transaction >60s`);
      console.error(`[health] WARNING: ${stuckCount} stuck idle-in-transaction connections detected`);
    }
