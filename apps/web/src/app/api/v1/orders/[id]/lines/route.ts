...
import { addLineItem, addLineItemSchema } from '@oppsera/module-orders';

function extractOrderId(request: NextRequest): string {
  const parts = new URL(request.url).pathname.split('/');
  // /api/v1/orders/:id/lines → parts[-2]
  return parts[parts.length - 2]!;
}

// POST /api/v1/orders/:id/lines — add line item
export const POST = withMiddleware(
  async (request: NextRequest, ctx) => {
    ...
    const result = await addLineItem(ctx, orderId, parsed.data);
