...
const updateBody = z.object({
  ...
  posOverridePin: z.string().regex(/^\d{4,8}$/, 'Override PIN must be 4-8 digits').optional().or(z.literal('')),
  uniqueIdentificationPin: z.string().regex(/^\d{4}$/, 'Login PIN must be exactly 4 digits').optional().or(z.literal('')),
