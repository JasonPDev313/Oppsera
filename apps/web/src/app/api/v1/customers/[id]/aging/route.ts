...
import { getCustomerAgingSummary } from '@oppsera/module-customers';
...
// GET /api/v1/customers/:id/aging — customer aging summary
export const GET = withMiddleware(
  async (request: NextRequest, ctx) => {
    const customerId = extractCustomerId(request);
    const data = await getCustomerAgingSummary({ tenantId: ctx.tenantId, customerId });
