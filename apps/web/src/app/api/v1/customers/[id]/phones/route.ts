...
export const GET = withMiddleware(
  async (request: NextRequest, ctx) => {
    ...
    return NextResponse.json({ data: contacts.phones });
  },
...
export const POST = withMiddleware(
  async (request: NextRequest, ctx) => {
    ...
    const parsed = addCustomerPhoneSchema.safeParse({ ...body, customerId });
    if (!parsed.success) {
    ...
    const phone = await addCustomerPhone(ctx, parsed.data);
    return NextResponse.json({ data: phone }, { status: 201 });
  },
