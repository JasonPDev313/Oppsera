...
export const membershipAccountingSettings = pgTable(
  'membership_accounting_settings',
  {
    ...
    defaultDuesReceivableAccountId: text('default_dues_receivable_account_id'),
    defaultHouseAccountReceivableAccountId: text('default_house_account_receivable_account_id'),
    defaultInitiationDeferredRevenueAccountId: text('default_initiation_deferred_revenue_account_id'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
