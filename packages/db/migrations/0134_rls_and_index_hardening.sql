...
DROP POLICY IF EXISTS ar_invoice_lines_select ON ar_invoice_lines;
CREATE POLICY ar_invoice_lines_select ON ar_invoice_lines
  FOR SELECT USING (
    tenant_id = (SELECT current_setting('app.current_tenant_id', true))
  );

DROP POLICY IF EXISTS ar_invoice_lines_insert ON ar_invoice_lines;
CREATE POLICY ar_invoice_lines_insert ON ar_invoice_lines
  FOR INSERT WITH CHECK (
    tenant_id = (SELECT current_setting('app.current_tenant_id', true))
  );

DROP POLICY IF EXISTS ar_invoice_lines_update ON ar_invoice_lines;
CREATE POLICY ar_invoice_lines_update ON ar_invoice_lines
  FOR UPDATE USING (
    tenant_id = (SELECT current_setting('app.current_tenant_id', true))
  );

DROP POLICY IF EXISTS ar_invoice_lines_delete ON ar_invoice_lines;
...
DROP POLICY IF EXISTS pje_select ON payment_journal_entries;
CREATE POLICY pje_select ON payment_journal_entries
  FOR SELECT USING (
    tenant_id = (SELECT current_setting('app.current_tenant_id', true))
  );

DROP POLICY IF EXISTS pje_insert ON payment_journal_entries;
...
CREATE INDEX IF NOT EXISTS idx_ap_bill_lines_bill
...
CREATE INDEX IF NOT EXISTS idx_ar_invoice_lines_account
  ON ar_invoice_lines (account_id);

-- 4e. ar_invoice_lines.tenant_id (for RLS + query performance)
CREATE INDEX IF NOT EXISTS idx_ar_invoice_lines_tenant
  ON ar_invoice_lines (tenant_id);

-- 4f-4j: Wrap remaining indexes in DO block to skip gracefully if tables/columns don't exist
DO $$ BEGIN
  -- 4f. item_uom_conversions FK indexes
  CREATE INDEX IF NOT EXISTS idx_item_uom_conversions_item
    ON item_uom_conversions (inventory_item_id);
EXCEPTION WHEN undefined_table OR undefined_column THEN NULL;
END $$;

DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_item_uom_conversions_uom
...
DO $$ BEGIN
  ...
  CREATE INDEX IF NOT EXISTS idx_item_vendors_vendor
...
DO $$ BEGIN
  ...
  CREATE INDEX IF NOT EXISTS idx_customer_relationships_related
...
DO $$ BEGIN
  ...
  CREATE INDEX IF NOT EXISTS idx_chargebacks_provider_case
