...
export async function hostGetDashboardMetrics(
  ...
  return withTenant(input.tenantId, async (tx) => {
    const [coverRows, tableRows, waitRows, reservationRows] = await Promise.all([
      // Covers: seated + completed (coversSeated) and all non-canceled/no-show (coversExpected)
      tx.execute(sql`
        SELECT
          COALESCE(
      ...
      // Waitlist: avg wait + active count
      tx.execute(sql`
        SELECT
          COALESCE(
      ...
      // Reservations: remaining (booked/confirmed) + no-shows
      tx.execute(sql`
        SELECT
