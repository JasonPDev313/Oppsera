  ...
  return withTenant(input.tenantId, async (tx) => {
    ...
    const ticketRows = await tx.execute(
      sql`SELECT id, ticket_number, tab_id, course_number, status,
                 table_number, server_name, sent_at,
    ...
    for (const t of tickets) {
      const itemRows = await tx.execute(
        sql`SELECT kti.id, kti.item_name, kti.modifier_summary, kti.seat_number,
                   kti.course_name, kti.quantity, kti.item_status, kti.station_id,
