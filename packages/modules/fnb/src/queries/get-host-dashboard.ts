...
export async function getHostDashboard(
  ...
  const emptyDashboard: HostDashboard = {
    ...
    tableSummary: { total: 0, available: 0, seated: 0, reserved: 0, dirty: 0, blocked: 0 },
    servers: [],
    nextUpServerUserId: null,
    stats: {
      totalCoversToday: 0,
      currentWaiting: 0,
      avgWaitMinutes: 0,
      reservationsToday: 0,
      noShowsToday: 0,
      seatedFromWaitlist: 0,
    },
  };

  try {
    return await withTenant(input.tenantId, async (tx) => {
      // Each query catches independently so a missing table doesn't crash the whole dashboard
      ...
      const [waitlistRows, reservationRows, tableRows, serverRows, statsRows, rotationRows] = await Promise.all([
        ...
        // Daily stats — two separate queries to avoid cross-table subquery failure
        safeQuery(tx.execute(sql`
          SELECT
      ...
      // Reservation count as separate safe query (was a correlated subquery before)
      const reservationCountRows = await safeQuery(tx.execute(sql`
        SELECT COUNT(*)::int AS reservations_today
        FROM fnb_reservations
        WHERE tenant_id = ${input.tenantId}
          AND location_id = ${input.locationId}
          AND reservation_date = ${input.businessDate}
      `));
      ...
      const rc = Array.from(reservationCountRows as Iterable<Record<string, unknown>>)[0] ?? {};
      const stats = {
        totalCoversToday: Number(st.seated_covers ?? 0),
        currentWaiting: Number(st.current_waiting ?? 0),
        avgWaitMinutes: Number(st.avg_wait ?? 0),
        reservationsToday: Number(rc.reservations_today ?? 0),
    ...
    console.error('[getHostDashboard] Query failed — returning empty dashboard:', err);
