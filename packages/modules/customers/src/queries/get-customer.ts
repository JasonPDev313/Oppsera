...
export async function getCustomer(input: GetCustomerInput): Promise<CustomerDetail> {
  return withTenant(input.tenantId, async (tx) => {
    ...
    // Fetch identifiers, activity log, memberships, and billing account links in parallel
    const [identifiers, activityLog, membershipRows, primaryAccounts, memberAccountLinks] = await Promise.all([
      tx
        .select()
