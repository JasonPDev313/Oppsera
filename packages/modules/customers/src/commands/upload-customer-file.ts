...
import type { UploadCustomerFileInput } from '../validation';

export async function uploadCustomerFile(ctx: RequestContext, input: UploadCustomerFileInput) {
  const result = await publishWithOutbox(ctx, async (tx) => {
    ...
    await (tx as any).insert(customerActivityLog).values({
      ...
      customerId: input.customerId,
      activityType: 'system',
      title: `File uploaded: ${input.documentType} - ${input.name}`,
      metadata: {
    ...
    const event = buildEventFromContext(ctx, 'customer.file.uploaded.v1', {
      customerId: input.customerId,
      documentId: created!.id,
      documentType: input.documentType,
      name: input.name,
  ...
  auditLogDeferred(ctx, 'customer.file_uploaded', 'customer', input.customerId);
  return result;
}
