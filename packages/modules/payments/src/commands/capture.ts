  ...
  const result = await publishWithOutbox(ctx, async (tx) => {
    ...
    if (existingCaptureTxn && existingCaptureTxn.responseStatus === 'approved') {
      const [currentIntent] = await tx
        ...
        .limit(1);
      if (currentIntent) {
        return { result: mapIntentToResult(currentIntent, existingCaptureTxn.providerRef, null), events: [] };
      }
    }
    ...
    // 5. Call provider capture (provider resolved outside transaction)
    ...
    if (captureResponse.status === 'approved') {
      ...
      return { result: mapIntentToResult(updated!, captureResponse.providerRef, interpretation), events: [event] };
    }

    return { result: mapIntentToResult(updated!, captureResponse.providerRef, interpretation), events: [] };
  });

  auditLogDeferred(ctx, 'payment.captured', 'payment_intent', result.id);
  return result;
}
