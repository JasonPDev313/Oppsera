...
- **IMPORTANT: Use mode="sql" for customer spend by period** — questions about "average spend per customer" with any time dimension (trending, MoM, YoY, by month, by week). The customer metrics (avg_customer_spend, total_customer_spend) use rm_customer_activity which stores lifetime running totals with NO date dimension. SQL mode calculates period-based customer spend directly from the orders table.
