...
const TABLE_DESCRIPTIONS: Record<string, string> = {
  ...
  // Spa (Spa & Wellness)
  spa_settings: 'Per-tenant spa configuration (timezone, buffer/cleanup minutes, booking toggles, deposit rules)',
  spa_service_categories: 'Hierarchical spa service categories (massage, facial, body, nail, etc.)',
  spa_services: 'Spa service definitions (name, duration, price in DOLLARS, member price, peak price, capacity, couples/group flags)',
  spa_service_addons: 'Spa add-ons (aromatherapy, hot stones, etc.) with price in DOLLARS',
  spa_service_addon_links: 'Many-to-many between services and addons (isDefault, priceOverride)',
  spa_service_resource_requirements: 'Resources required by each service (room type, equipment)',
  spa_providers: 'Spa therapists/providers (display name, specialties JSONB, certifications, employment type, max daily appointments)',
  spa_provider_availability: 'Provider weekly schedule template (day of week, start/end time)',
  spa_provider_time_off: 'Provider time-off requests (status: pending/approved/rejected)',
  spa_provider_service_eligibility: 'Which services a provider can perform (proficiency level, custom pricing)',
  spa_resources: 'Spa rooms, beds, chairs, equipment (resource type, capacity, amenities)',
  spa_appointments: 'Spa appointments (status, provider, resource, customer, times, deposit, booking source, order link). Prices in CENTS. Status: draft→reserved→confirmed→checked_in→in_service→completed→checked_out | canceled | no_show',
  spa_appointment_items: 'Individual services/addons within an appointment (prices in CENTS, per-item status)',
  spa_appointment_history: 'Appointment audit trail (status transitions, changes JSONB)',
  spa_intake_form_templates: 'Intake/consent form definitions (fields JSONB)',
  spa_intake_responses: 'Customer intake form responses per appointment',
  spa_clinical_notes: 'SOAP clinical notes per appointment (subjective, objective, assessment, plan)',
  spa_contraindications: 'Customer health flags affecting service eligibility',
  spa_waitlist: 'Spa waiting list entries (status: waiting/offered/booked/expired/canceled)',
  spa_commission_rules: 'Provider commission rules (percentage/flat/tiered, per service or category)',
  spa_commission_ledger: 'Per-appointment commission records (amounts in CENTS, status: calculated/approved/paid)',
  spa_package_definitions: 'Spa package products (session bundles, credit bundles, validity, pricing in CENTS)',
  spa_package_balances: 'Customer-owned package instances (sessions/credits remaining, status)',
  spa_package_redemptions: 'Package redemption records per appointment',
  spa_room_turnover_tasks: 'Room prep/cleanup tasks (status: pending/in_progress/completed/skipped)',
  spa_daily_operations: 'Daily open/close tracking per location (checklists, incidents)',
  spa_booking_widget_config: 'Online booking widget customization (theme, branding, policies)',

  // Spa read models (CQRS)
  rm_spa_daily_operations: 'Daily spa operations aggregate per location (appointment counts, revenue in DOLLARS, utilization %, rebooking rate)',
  rm_spa_provider_metrics: 'Daily spa provider performance per provider (appointments, revenue in DOLLARS, commissions, tips, utilization, client counts)',
  rm_spa_service_metrics: 'Daily spa service analytics per service (bookings, revenue in DOLLARS, addon attachment rate)',
  rm_spa_client_metrics: 'Daily spa client activity per customer (visits, spend in DOLLARS, services, packages, cancels, no-shows)',
...
const COLUMN_DESCRIPTIONS: Record<string, Record<string, string>> = {
  ...
  spa_services: {
    price: 'service price in DOLLARS (NUMERIC)',
    member_price: 'discounted price for members in DOLLARS (NUMERIC, nullable)',
    peak_price: 'peak/weekend price in DOLLARS (NUMERIC, nullable)',
    cost: 'cost to business in DOLLARS (NUMERIC, nullable)',
    duration_minutes: 'standard service duration in minutes',
    buffer_minutes: 'buffer time between appointments',
    category: 'massage|facial|body|nail|hair|wellness|medspa|other',
    is_active: 'true if service is bookable',
    archived_at: 'non-NULL if service is archived (soft delete)',
    max_capacity: 'max simultaneous clients (1 for solo, >1 for group)',
  },
  spa_appointments: {
    ...
    start_at: 'appointment start time (timestamptz)',
    end_at: 'appointment end time (timestamptz)',
    deposit_amount_cents: 'deposit amount in CENTS (INTEGER)',
    deposit_status: 'none|required|authorized|captured|refunded',
    provider_id: 'FK to spa_providers — assigned therapist',
    resource_id: 'FK to spa_resources — assigned room/bed',
    customer_id: 'FK to customers — NULL for guest walk-ins (use guest_name instead)',
    guest_name: 'walk-in guest name when no customer record',
    order_id: 'FK to orders — linked POS order for checkout',
    pms_folio_id: 'FK to pms_folios — linked hotel folio for resort spa',
    appointment_number: 'unique human-readable appointment number per tenant',
  },
  spa_appointment_items: {
    price_cents: 'listed price in CENTS',
    final_price_cents: 'final price after discounts in CENTS',
  ...
  spa_providers: {
    ...
    is_bookable_online: 'true if provider appears in online booking widget',
    max_daily_appointments: 'daily appointment cap',
    color: 'hex color for calendar display',
  },
  spa_commission_ledger: {
    base_amount_cents: 'service revenue in CENTS that commission is calculated on',
    commission_amount_cents: 'commission earned in CENTS',
    rate_applied: 'commission rate that was applied (NUMERIC)',
    status: 'calculated|approved|paid|adjusted|voided',
    pay_period: 'pay period identifier string',
  },
  spa_package_definitions: {
    selling_price_cents: 'package selling price in CENTS',
    total_value_cents: 'total value of included services in CENTS',
    package_type: 'session_bundle|credit_bundle|time_bundle|value_bundle',
    validity_days: 'days from purchase until expiration',
  },
  spa_package_balances: {
    sessions_total: 'total sessions in package',
    sessions_used: 'sessions redeemed so far',
    credits_total: 'total credits in package',
    credits_used: 'credits redeemed so far',
    status: 'active|frozen|expired|exhausted|canceled',
  },
  rm_spa_daily_operations: {
    business_date: 'aggregation date YYYY-MM-DD (one row per location per date)',
    appointment_count: 'total appointments scheduled for this date',
    completed_count: 'appointments completed',
    canceled_count: 'appointments canceled',
    no_show_count: 'appointment no-shows',
    walk_in_count: 'walk-in appointments',
    online_booking_count: 'online bookings',
    total_revenue: 'total spa revenue in DOLLARS (NUMERIC)',
    service_revenue: 'revenue from services in DOLLARS',
    addon_revenue: 'revenue from add-ons in DOLLARS',
    retail_revenue: 'revenue from retail products in DOLLARS',
    tip_total: 'total tips in DOLLARS',
    avg_appointment_duration: 'average appointment duration in minutes',
    utilization_pct: 'provider utilization percentage (NUMERIC 5,2)',
    rebooking_rate: 'rebooking rate percentage (NUMERIC 5,2)',
  },
  rm_spa_provider_metrics: {
    business_date: 'aggregation date YYYY-MM-DD (one row per provider per date)',
    provider_id: 'FK to spa_providers',
    appointment_count: 'appointments for this provider on this date',
    completed_count: 'completed appointments',
    total_revenue: 'provider revenue in DOLLARS (NUMERIC)',
    commission_total: 'commissions earned in DOLLARS (NUMERIC)',
    tip_total: 'tips earned in DOLLARS (NUMERIC)',
    utilization_pct: 'provider utilization percentage',
    rebooking_rate: 'provider rebooking rate',
    client_count: 'distinct clients seen',
    new_client_count: 'new clients seen',
  },
  rm_spa_service_metrics: {
    business_date: 'aggregation date YYYY-MM-DD (one row per service per date)',
    service_id: 'FK to spa_services',
    booking_count: 'number of times this service was booked',
    completed_count: 'completed bookings for this service',
    total_revenue: 'service revenue in DOLLARS (NUMERIC)',
    avg_price_cents: 'average price charged in CENTS',
    addon_attachment_rate: 'percentage of bookings that added add-ons',
  },
  rm_spa_client_metrics: {
    business_date: 'aggregation date YYYY-MM-DD (one row per customer per date)',
    customer_id: 'FK to customers',
    visit_count: 'spa visits on this date',
    total_spend: 'total spend in DOLLARS (NUMERIC)',
    service_count: 'services consumed',
    cancel_count: 'cancellations by this client',
    no_show_count: 'no-shows by this client',
    tip_total: 'tips given in DOLLARS (NUMERIC)',
  },
};
