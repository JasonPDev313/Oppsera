...
import { createAdminClient, loginRecords, adminLoginRecords, guardedQuery } from '@oppsera/db';
...
export async function recordLoginEvent(params: RecordLoginParams): Promise<string | null> {
  try {
    ...
    await guardedQuery('login_record.insert', () =>
      adminDb.insert(loginRecords).values({
        id,
...
export async function recordAdminLoginEvent(params: RecordAdminLoginParams): Promise<void> {
  try {
    const adminDb = createAdminClient();
    await guardedQuery('admin_login_record.insert', () =>
      adminDb.insert(adminLoginRecords).values({
        id: generateUlid(),
...
export async function stampLoginTerminal(
  ...
  try {
    ...
    const rows = await guardedQuery('login_record.find_for_terminal', () =>
      adminDb
    ...
    await guardedQuery('login_record.stamp_terminal', () =>
      adminDb
        .update(loginRecords)
