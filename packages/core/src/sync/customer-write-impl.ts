import { withTenant, customers, customerExternalIds, customerActivityLog } from '@oppsera/db';
import { eq, and } from 'drizzle-orm';
import { generateUlid } from '@oppsera/shared';
import type { CustomerWriteApi, EnsureCustomerResult } from '../helpers/customer-write-api';
import { logger } from '../observability';

type ComputeDisplayName = (input: {
  type: string;
  firstName?: string;
  lastName?: string;
  email?: string | null;
...
/**
 * Creates a CustomerWriteApi implementation backed by the DB.
 *
 * `computeDisplayName` is injected to avoid importing @oppsera/module-customers
 * from core (which would violate the dependency rules — modules depend on core,
 * not the other way around).
 */
export function createCustomerWriteApiImpl(computeDisplayName: ComputeDisplayName): CustomerWriteApi {
  return {
    async ensureCustomer(ctx, input): Promise<EnsureCustomerResult | null> {
      try {
        ...
        logger.error('[CustomerWriteApi] ensureCustomer failed', {
