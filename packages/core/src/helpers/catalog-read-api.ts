...
class DrizzleCatalogReadApi implements CatalogReadApi {
  ...
  async getItemForPOS(
    ...
    // 4 separate withTenant calls (getItem + getEffectivePrice + getItemTaxes +
    // getSubDepartmentForItem). Under the old pattern, a batch of N items used
    // 4N semaphore slots against a limit of 3, causing instant pool exhaustion
    // and circuit breaker trips on concurrent POS requests.
    return withTenant(tenantId, async (tx) => {
      // ── getItem ──
      const [item] = await tx
        .select()
      ...
      // ── parallel: effectivePrice + taxInfo + subDepartment (same tx, no extra semaphore slots) ──
